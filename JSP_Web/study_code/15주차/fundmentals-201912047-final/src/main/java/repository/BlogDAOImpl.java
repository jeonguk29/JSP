...
public class BlogDAOImpl extends OracleDAO implements BlogDAO {
    ...
    public Blog read(Blog b) {
        Blog ret = null;
        String sql = "select * from blog where id ='" + b.getId() + "'";
        try {
            conn =  getConnection();
    ...
    public int update(Blog b) {
        int ret = 0;
        String sql = "update blog set title =? , content=?, regdate=?, author=? where email =?"; // ? : placeholder 자리 대체자
        //sSystem.out.println(sql);
        try {
            conn = getConnection(); // DB 연결 객체 생성
    ...
    public int delete(Blog blog) {
        int ret = 0;
        String sql = "delete from blog where id = ?"; // ? : placeholder 자리 대체자
        //sSystem.out.println(sql);
        try {
            conn = getConnection(); // DB 연결 객체 생성
